...
static int send_state(int client_fd, const GameState *game)
{
    ...
    if (append_state_int(state_line, sizeof(state_line), &offset, game->phase) ||
        ...
        append_state_int(state_line, sizeof(state_line), &offset, game->p1_reveal_ms) ||
        append_state_int(state_line, sizeof(state_line), &offset, game->p2_reveal_ms)) {
        return -1;
    }
    ...
    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        if (append_state_int(state_line, sizeof(state_line), &offset, game->p1_ult_bullet_row[index]) ||
            append_state_int(state_line, sizeof(state_line), &offset, game->p1_ult_bullet_col[index]) ||
            append_state_int(state_line, sizeof(state_line), &offset, game->p1_ult_bullet_active[index]) ||
            append_state_int(state_line, sizeof(state_line), &offset, game->p2_ult_bullet_row[index]) ||
            append_state_int(state_line, sizeof(state_line), &offset, game->p2_ult_bullet_col[index]) ||
            append_state_int(state_line, sizeof(state_line), &offset, game->p2_ult_bullet_active[index])) {
            return -1;
        }
    }
...
static void set_player_ultimate_type(GameState *game, int player_id, int ult_type)
{
    Player *player = player_id == 1 ? &game->p1 : &game->p2;

    if (ult_type < ULT_ONE_SHOT || ult_type > ULT_REVEAL) {
        return;
    }

    player_set_action(player, ACTION_ULTIMATE);
    player->ultimate_type = ult_type;

    if (ult_type == ULT_REVEAL) {
        game_start_reveal_reposition(game, player_id);
    } else {
        game_clear_reveal_reposition(game, player_id);
    }
}

static int ultimate_type_from_key(int key)
{
    if (key == KEY_ULT_ONE) return ULT_ONE_SHOT;
    if (key == KEY_ULT_TWO) return ULT_BARRAGE;
    if (key == KEY_ULT_THREE) return ULT_DEFLECT;
    if (key == KEY_ULT_FOUR) return ULT_REVEAL;
    return ULT_NONE;
}

static int ultimate_type_from_command(const char *line)
{
    if (strcmp(line, "ULT1") == 0) return ULT_ONE_SHOT;
    if (strcmp(line, "ULT2") == 0) return ULT_BARRAGE;
    if (strcmp(line, "ULT3") == 0) return ULT_DEFLECT;
    if (strcmp(line, "ULT4") == 0) return ULT_REVEAL;
    return ULT_NONE;
}
...
static int handle_local_input(GameState *game, int key, int *quit_armed)
{
    ...
    *quit_armed = 0;
    ...
    if (game->phase == PHASE_WAITING) {
        ...
        int selected_ult = ultimate_type_from_key(key);

        if (game->p1.action == ACTION_ULTIMATE) {
            if (selected_ult != ULT_NONE && game->p1.ultimate_ready) {
                set_player_ultimate_type(game, 1, selected_ult);
            } else if (game->p1.ultimate_type == ULT_REVEAL && key == KEY_LEFT) {
                player_move(&game->p1, -1, 0, GRID_WIDTH - 1);
            } else if (game->p1.ultimate_type == ULT_REVEAL && key == KEY_RIGHT) {
                player_move(&game->p1, 1, 0, GRID_WIDTH - 1);
            } else if (key == KEY_ULTIMATE) {
                game_clear_reveal_reposition(game, 1);
            ...
            if (key == KEY_LEFT || key == KEY_SELECT_SHOOT) {
                ...
                player_set_action(&game->p1, ACTION_HEAL);
            } else if (selected_ult != ULT_NONE && game->p1.ultimate_ready) {
                set_player_ultimate_type(game, 1, selected_ult);
                ...
                if (game->p1.ultimate_type == ULT_REVEAL)
                    game_start_reveal_reposition(game, 1);
            } else if (key == KEY_SPACE) {
                player_lock(&game->p1);
            }
        }
...
static void handle_remote_command(GameState *game, const char *line)
{
    ...
    if (game->phase == PHASE_WAITING) {
        ...
        int selected_ult = ultimate_type_from_command(line);

        if (game->p2.action == ACTION_ULTIMATE) {
            if (selected_ult != ULT_NONE && game->p2.ultimate_ready) {
                set_player_ultimate_type(game, 2, selected_ult);
            } else if (game->p2.ultimate_type == ULT_REVEAL && strcmp(line, "LEFT") == 0) {
                player_move(&game->p2, -1, 0, GRID_WIDTH - 1);
            } else if (game->p2.ultimate_type == ULT_REVEAL && strcmp(line, "RIGHT") == 0) {
                player_move(&game->p2, 1, 0, GRID_WIDTH - 1);
            } else if (strcmp(line, "ULTIMATE") == 0) {
                game_clear_reveal_reposition(game, 2);
            ...
            if (strcmp(line, "LEFT") == 0 || strcmp(line, "SHOOT") == 0) {
                ...
                player_set_action(&game->p2, ACTION_HEAL);
            } else if (selected_ult != ULT_NONE && game->p2.ultimate_ready) {
                set_player_ultimate_type(game, 2, selected_ult);
                ...
                if (game->p2.ultimate_type == ULT_REVEAL)
                    game_start_reveal_reposition(game, 2);
            } else if (strcmp(line, "LOCK") == 0) {
                player_lock(&game->p2);
            }
        }
...
static void update_match(GameState *game, int delta_ms)
{
    ...
    if (game->phase == PHASE_ACTION) {
        if (game->p1_reveal_ms > 0 && !game->p1.locked) {
            game->p1_reveal_ms -= delta_ms;
            if (game->p1_reveal_ms <= 0) {
                game->p1_reveal_ms = 0;
                player_lock(&game->p1);
            }
        }

        if (game->p2_reveal_ms > 0 && !game->p2.locked) {
            game->p2_reveal_ms -= delta_ms;
            if (game->p2_reveal_ms <= 0) {
                game->p2_reveal_ms = 0;
                player_lock(&game->p2);
            }
        }

        if ((game->p1.locked && game->p2.locked) || game->phase_time_ms >= (game->p1.ultimate_ready || game->p2.ultimate_ready || game->p1.action == ACTION_ULTIMATE || game->p2.action == ACTION_ULTIMATE ? ACTION_PHASE_ULTIMATE_MS : ACTION_PHASE_MS)) {
    ...
    if (game->phase == PHASE_RESOLVE) {
        ...
        if (game->phase_time_ms >= RESOLVE_PHASE_MS && !game_has_active_resolve_projectiles(game)) {
            if (game->winner != 0) {
                game->phase = PHASE_GAME_OVER;
...
int start_server(int port)
{
    ...
    char render_key[2048];
    char last_render_key[2048];
