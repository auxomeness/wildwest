...
#define READ_STATE_INT(target)       \
    ...
    READ_STATE_INT(game->p1_reveal_ms);
    READ_STATE_INT(game->p2_reveal_ms);
    ...
    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        READ_STATE_INT(game->p1_ult_bullet_row[index]);
        READ_STATE_INT(game->p1_ult_bullet_col[index]);
        READ_STATE_INT(game->p1_ult_bullet_active[index]);
        READ_STATE_INT(game->p2_ult_bullet_row[index]);
        READ_STATE_INT(game->p2_ult_bullet_col[index]);
        READ_STATE_INT(game->p2_ult_bullet_active[index]);
    }
...
int main(int argc, char **argv)
{
    ...
    while (keep_running) {
        ...
        while (key_available()) {
            ...
            } else if (key == KEY_ULT_ONE) {
                if (send_text(socket_fd, "ULT1\n") != 0) {
                    keep_running = 0;
                    break;
                }
            } else if (key == KEY_ULT_TWO) {
                if (send_text(socket_fd, "ULT2\n") != 0) {
                    keep_running = 0;
                    break;
                }
            } else if (key == KEY_ULT_THREE) {
                if (send_text(socket_fd, "ULT3\n") != 0) {
                    keep_running = 0;
                    break;
                }
            } else if (key == KEY_ULT_FOUR) {
                if (send_text(socket_fd, "ULT4\n") != 0) {
                    keep_running = 0;
                    break;
                }
