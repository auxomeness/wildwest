...
static void clear_bullets(GameState *game)
{
    ...
    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        game->p1_ult_bullet_row[index] = 0;
        game->p1_ult_bullet_col[index] = 0;
        game->p1_ult_bullet_active[index] = 0;
        game->p2_ult_bullet_row[index] = 0;
        game->p2_ult_bullet_col[index] = 0;
        game->p2_ult_bullet_active[index] = 0;
    }
}
...
static int local_reveal_active(const GameState *game, int player_id)
{
    const Player *self = local_player(game, player_id);

    return game->phase == PHASE_ACTION &&
           self->ultimate_ready &&
           self->action == ACTION_ULTIMATE &&
           self->ultimate_type == ULT_REVEAL;
}

static int action_time_limit(const GameState *game)
{
    if (game->p1.ultimate_ready || game->p2.ultimate_ready ||
        game->p1.action == ACTION_ULTIMATE || game->p2.action == ACTION_ULTIMATE) {
        return ACTION_PHASE_ULTIMATE_MS;
    }

    return ACTION_PHASE_MS;
}

static int reveal_time_remaining(const GameState *game, int player_id)
{
    return player_id == 1 ? game->p1_reveal_ms : game->p2_reveal_ms;
}

void game_start_reveal_reposition(GameState *game, int player_id)
{
    int *timer = player_id == 1 ? &game->p1_reveal_ms : &game->p2_reveal_ms;

    if (*timer <= 0) {
        *timer = REVEAL_REPOSITION_MS;
    }
}

void game_clear_reveal_reposition(GameState *game, int player_id)
{
    int *timer = player_id == 1 ? &game->p1_reveal_ms : &game->p2_reveal_ms;
    *timer = 0;
}
...
static void setup_barrage_bullets(GameState *game, int player_id)
{
    int used[GRID_WIDTH] = {0};
    int *rows = player_id == 1 ? game->p1_ult_bullet_row : game->p2_ult_bullet_row;
    int *cols = player_id == 1 ? game->p1_ult_bullet_col : game->p2_ult_bullet_col;
    int *active = player_id == 1 ? game->p1_ult_bullet_active : game->p2_ult_bullet_active;
    int start_row = player_id == 1 ? current_player1_row(game) + 1 : current_player2_row(game) - 1;
    int index;
    ...
    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        ...
        do {
            lane = rand() % GRID_WIDTH;
        } while (used[lane]);

        used[lane] = 1;
        rows[index] = start_row;
        cols[index] = lane;
        active[index] = 1;
    }
}

static int barrage_hits_col(const int *cols, const int *active, int target_col)
{
    int index;

    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        if (active[index] && cols[index] == target_col) {
            return 1;
        }
    }
...
static void apply_damage(Player *target, int damage)
{
    target->hp -= damage;
    if (target->hp < 0) {
        target->hp = 0;
...
void game_start_resolve_phase(GameState *game)
{
    ...
    if (game->p1.action == ACTION_SHOOT ||
        (game->p1.action == ACTION_ULTIMATE && game->p1.ultimate_type == ULT_ONE_SHOT)) {
        game->bullet1_active = 1;
        game->bullet1_row = current_player1_row(game) + 1;
        game->bullet1_col = game->p1.col;
        game->p1_bullet_step_ms = 0;
    } else if (game->p1.action == ACTION_ULTIMATE && game->p1.ultimate_type == ULT_BARRAGE) {
        setup_barrage_bullets(game, 1);
    }

    if (game->p2.action == ACTION_SHOOT ||
        (game->p2.action == ACTION_ULTIMATE && game->p2.ultimate_type == ULT_ONE_SHOT)) {
        game->bullet2_active = 1;
        game->bullet2_row = current_player2_row(game) - 1;
        game->bullet2_col = game->p2.col;
        game->p2_bullet_step_ms = 0;
    } else if (game->p2.action == ACTION_ULTIMATE && game->p2.ultimate_type == ULT_BARRAGE) {
        setup_barrage_bullets(game, 2);
    }
    ...
    /* Deflect resolves before offensive ultimates so it can counter One Shot. */
    if (game->p1.action == ACTION_ULTIMATE &&
        game->p1.ultimate_type == ULT_DEFLECT &&
        ultimate_can_use(&game->p1)) {
    ...
    if (game->p2.action == ACTION_ULTIMATE &&
        game->p2.ultimate_type == ULT_DEFLECT &&
        ultimate_can_use(&game->p2)) {
        ultimate_execute(&game->p2, &game->p1);
        game->p2_hit_streak = 0;
        game->p2_ult_result = RESULT_ULT_DEFLECT;
    }

    /* Execute remaining ultimates before normal shots resolve. */
    if (game->p1.action == ACTION_ULTIMATE && ultimate_can_use(&game->p1)) {
        int ult = game->p1.ultimate_type;
        ultimate_execute(&game->p1, &game->p2);
        game->p1_hit_streak = 0;
        if (ult == ULT_ONE_SHOT) {
            if (game->p2.is_deflecting && game->p1.col == game->p2.col) {
                game->p1_ult_result = RESULT_ULT_MISS;
            } else {
                game->p1_ult_result = (game->p1.col == game->p2.col) ? RESULT_ULT_HIT : RESULT_ULT_MISS;
            }
        } else if (ult == ULT_BARRAGE) {
            if (barrage_hits_col(game->p1_ult_bullet_col, game->p1_ult_bullet_active, game->p2.col)) {
                if (game->p2.is_deflecting) {
                    apply_damage(&game->p1, 20);
                    game->p1_ult_result = RESULT_ULT_MISS;
                } else {
                    apply_damage(&game->p2, 35);
                    game->p1_ult_result = RESULT_ULT_HIT;
                }
            } else {
                game->p1_ult_result = RESULT_ULT_MISS;
            }
    ...
    if (game->p2.action == ACTION_ULTIMATE && ultimate_can_use(&game->p2)) {
        ...
        game->p2_hit_streak = 0;
        if (ult == ULT_ONE_SHOT) {
            if (game->p1.is_deflecting && game->p2.col == game->p1.col) {
                game->p2_ult_result = RESULT_ULT_MISS;
            } else {
                game->p2_ult_result = (game->p2.col == game->p1.col) ? RESULT_ULT_HIT : RESULT_ULT_MISS;
            }
        } else if (ult == ULT_BARRAGE) {
            if (barrage_hits_col(game->p2_ult_bullet_col, game->p2_ult_bullet_active, game->p1.col)) {
                if (game->p1.is_deflecting) {
                    apply_damage(&game->p2, 20);
                    game->p2_ult_result = RESULT_ULT_MISS;
                } else {
                    apply_damage(&game->p1, 35);
                    game->p2_ult_result = RESULT_ULT_HIT;
                }
            } else {
                game->p2_ult_result = RESULT_ULT_MISS;
            }
    ...
    /* Ultimate charge counts shoot actions, hit or miss. Healing is neutral. */
    if (!game->p1.ultimate_ready) {
        if (game->p1.action == ACTION_SHOOT) {
            game->p1_hit_streak++;
        }
        if (game->p1_hit_streak >= 3) {
...
void game_update_bullets(GameState *game, int delta_ms)
{
    ...
    {
        int index;
        int p1_projectile_active = game->bullet1_active;
        int p2_projectile_active = game->bullet2_active;
        int p1_steps = 0;
        int p2_steps = 0;
        int p1_ult_steps;
        int p2_ult_steps;
        ...
        for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
            if (game->p1_ult_bullet_active[index]) {
                p1_projectile_active = 1;
            }
            if (game->p2_ult_bullet_active[index]) {
                p2_projectile_active = 1;
            }
        }

        if (p1_projectile_active) {
            game->p1_bullet_step_ms += delta_ms;
            while (game->p1_bullet_step_ms >= RESOLVE_BULLET_STEP_MS) {
                game->p1_bullet_step_ms -= RESOLVE_BULLET_STEP_MS;
                p1_steps++;
            }
        }

        if (p2_projectile_active) {
            game->p2_bullet_step_ms += delta_ms;
            while (game->p2_bullet_step_ms >= RESOLVE_BULLET_STEP_MS) {
                game->p2_bullet_step_ms -= RESOLVE_BULLET_STEP_MS;
                p2_steps++;
            }
        }

        p1_ult_steps = p1_steps;
        p2_ult_steps = p2_steps;

        while (game->bullet1_active && p1_steps > 0) {
            game->bullet1_row += 1;
            if (game->p2.is_deflecting &&
                game->bullet1_col == game->p2.col &&
                game->bullet1_row >= current_player2_row(game)) {
                game->bullet1_active = 0;
                start_reflect_bullet(game, 2, game->p2.col);
            } else if (game->bullet1_row > current_player2_row(game)) {
                game->bullet1_active = 0;
            }
            p1_steps--;
        }

        while (game->bullet2_active && p2_steps > 0) {
            game->bullet2_row -= 1;
            if (game->p1.is_deflecting &&
                game->bullet2_col == game->p1.col &&
                game->bullet2_row <= current_player1_row(game)) {
                game->bullet2_active = 0;
                start_reflect_bullet(game, 1, game->p1.col);
            } else if (game->bullet2_row < current_player1_row(game)) {
                game->bullet2_active = 0;
            }
            p2_steps--;
        }
        ...
        for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
            ...
            for (step = 0; step < p1_ult_steps && game->p1_ult_bullet_active[index]; step++) {
                game->p1_ult_bullet_row[index] += 1;
                if (game->p2.is_deflecting &&
                    game->p1_ult_bullet_col[index] == game->p2.col &&
                    game->p1_ult_bullet_row[index] >= current_player2_row(game)) {
                    game->p1_ult_bullet_active[index] = 0;
                    start_reflect_bullet(game, 2, game->p2.col);
                } else if (game->p1_ult_bullet_row[index] > current_player2_row(game)) {
                    game->p1_ult_bullet_active[index] = 0;
                }
            }

            for (step = 0; step < p2_ult_steps && game->p2_ult_bullet_active[index]; step++) {
                game->p2_ult_bullet_row[index] -= 1;
                if (game->p1.is_deflecting &&
                    game->p2_ult_bullet_col[index] == game->p1.col &&
                    game->p2_ult_bullet_row[index] <= current_player1_row(game)) {
                    game->p2_ult_bullet_active[index] = 0;
                    start_reflect_bullet(game, 1, game->p1.col);
                } else if (game->p2_ult_bullet_row[index] < current_player1_row(game)) {
                    game->p2_ult_bullet_active[index] = 0;
                }
            }
        }
...
int game_has_active_resolve_projectiles(const GameState *game)
{
    int index;

    if (game->bullet1_active || game->bullet2_active) {
        return 1;
    }

    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        if (game->p1_ult_bullet_active[index] || game->p2_ult_bullet_active[index]) {
            return 1;
        }
    }
...
{
    int limit = game->phase == PHASE_ACTION ? action_time_limit(game) : game_phase_time_limit(game->phase);
    int remaining = limit - game->phase_time_ms;
...
{
    switch (ult_type) {
        case ULT_ONE_SHOT: return "Same col = instant kill. Miss = -10 self.";
        case ULT_BARRAGE:  return "3 random lanes. 35 dmg. 3/5 hit.";
        case ULT_DEFLECT:  return "Reflect shot/One Shot if aligned.";
        case ULT_REVEAL:   return "Reveal enemy lane before locking.";
        default:           return "";
    }
}
...
void game_build_display_key(const GameState *game, int player_id, char *buffer, size_t buffer_size)
{
    ...
    int enemy_visible = (game->phase == PHASE_RESOLVE ||
                         ...
                         local_reveal_active(game, player_id));
    int countdown = -1;
    ...
    append_display_key_int(buffer,
                           buffer_size,
                           game->phase == PHASE_SUDDEN_DEATH_READY ? game->phase_time_ms / 300 : 0);
    append_display_key_int(buffer, buffer_size, game->p1.ultimate_ready);
    append_display_key_int(buffer, buffer_size, game->p2.ultimate_ready);
    append_display_key_int(buffer, buffer_size, game->p1.ultimate_type);
    append_display_key_int(buffer, buffer_size, game->p2.ultimate_type);
    append_display_key_int(buffer, buffer_size, game->p1_hit_streak);
    append_display_key_int(buffer, buffer_size, game->p2_hit_streak);
    append_display_key_int(buffer, buffer_size, game->p1_ult_result);
    append_display_key_int(buffer, buffer_size, game->p2_ult_result);
    append_display_key_int(buffer, buffer_size, game->p1_reveal_ms / 1000);
    append_display_key_int(buffer, buffer_size, game->p2_reveal_ms / 1000);
    ...
    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
        append_display_key_int(buffer, buffer_size, game->p1_ult_bullet_row[index]);
        append_display_key_int(buffer, buffer_size, game->p1_ult_bullet_col[index]);
        append_display_key_int(buffer, buffer_size, game->p1_ult_bullet_active[index]);
        append_display_key_int(buffer, buffer_size, game->p2_ult_bullet_row[index]);
        append_display_key_int(buffer, buffer_size, game->p2_ult_bullet_col[index]);
        append_display_key_int(buffer, buffer_size, game->p2_ult_bullet_active[index]);
    }
}
...
void game_render(const GameState *game, int player_id, int quit_armed)
{
    ...
    if (game->phase == PHASE_SUDDEN_DEATH_OFFER) {
        ...
        snprintf(left_line1,
                 ...
                 self_ready ? 'x' : ' ');
        {
            int self_shots = player_id == 1 ? game->p1_hit_streak : game->p2_hit_streak;
    ...
    if (game->phase == PHASE_WAITING) {
        ...
        if (self->ultimate_ready && self->action == ACTION_ULTIMATE) {
            if (self->ultimate_type == ULT_REVEAL) {
                snprintf(controls_line, sizeof(controls_line),
                         "Controls: 1-4 = choose ult | Left/Right = reposition | Space = lock");
            } else {
                snprintf(controls_line, sizeof(controls_line),
                         "Controls: 1-4 = choose ultimate | Space = lock | S/H = cancel");
            }
        } else if (self->ultimate_ready) {
            snprintf(controls_line, sizeof(controls_line),
                     "Controls: Left = shoot | Right = heal | 1-4 = ultimate | Space = lock");
        } else {
            snprintf(controls_line, sizeof(controls_line),
        ...
        for (row = 0; row < arena_height; row++) {
            ...
            for (col = 0; col < GRID_WIDTH; col++) {
                ...
                if (game->phase == PHASE_SUDDEN_DEATH_BATTLE) {
                    ...
                    for (index = 0; index < ULT_BARRAGE_BULLETS; index++) {
                        if (game->p1_ult_bullet_active[index] &&
                            row == game->p1_ult_bullet_row[index] &&
                            col == game->p1_ult_bullet_col[index]) {
                            cell = "  !!!  ";
                            cell_used = 1;
                            break;
                        }
                        ...
                        if (game->p2_ult_bullet_active[index] &&
                            row == game->p2_ult_bullet_row[index] &&
                            col == game->p2_ult_bullet_col[index]) {
                            cell = "  !!!  ";
                            cell_used = 1;
                            break;
                        }
    ...
    if (game->phase == PHASE_WAITING) {
        ...
        if (self->ultimate_ready) {
            if (self->action == ACTION_ULTIMATE) {
                ...
                printf("[1] One Shot   [2] Barrage   [3] Deflect   [4] Reveal\n");
                if (self->ultimate_type == ULT_REVEAL) {
                    printf("Enemy revealed. Reposition time: %d sec. Left/Right = move | Space = lock\n",
                           (reveal_time_remaining(game, player_id) + 999) / 1000);
                } else {
                    printf("Press 1-4 to choose directly | Space = lock | S = Shoot | H = Heal\n");
                }
            } else {
                printf("SHOOT [%c]      HEAL [%c]      ULTIMATE [ ]\n",
                       self->action == ACTION_SHOOT ? 'x' : ' ',
                       self->action == ACTION_HEAL ? 'x' : ' ');
                printf("Left = Shoot | Right = Heal | 1-4 = select Ultimate | Space = lock\n");
            }
        } else {
